...
class CommandServer(Networking.NetworkProtocol):
    TICKS_PER_SECOND = 60
    HOST_IP = "0.0.0.0"
    ...
    def send_message(self, target: Union[User, socket.socket], command: str, **parameters):
        if isinstance(target, User):
            target = target.socket
        self.send_packet(target, self.build_packet(command, **parameters))
