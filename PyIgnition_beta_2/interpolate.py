...
def InterpolateKeyframes(curframe, variables, keyframes):
    ...
    for i in range(len(keys)):  # Determine current keyframe and next one for this variable
        ...
        for i in range(len(keyframes)):
            try:
                frame = keyframes[i]
                # If the current keyframe has a keyed value for the current variable
                if (frame.variables[key] != None):
                    if frame.frame <= curframe:  # If its frame is below or equal to the current, it is the current keyframe
                        curkeyframe = i
                    # If this is the first keyframe with a frame higher than the current, it is the next keyframe
                    if (nextkeyframe == None) and (frame.frame > curframe):
                        nextkeyframe = i
            except KeyError:
                pass

        # If there is no next key frame, maintain the value specified by the current one
        if nextkeyframe == None or key == "interpolationtype":
            # (Also do this if it is an interpolation type variable; they should only change once their next keyframe has been reached
            ...
            if keyframes[nextkeyframe].variables['interpolationtype'] == "linear":
                finalvariables[key] = LinearInterpolateKeyframes(
                    curframe, keyframes[curkeyframe].frame, keyframes[nextkeyframe].frame, keyframes[curkeyframe].variables[key], keyframes[nextkeyframe].variables[key])
            elif keyframes[nextkeyframe].variables['interpolationtype'] == "cosine":
                finalvariables[key] = CosineInterpolateKeyframes(
                    curframe, keyframes[curkeyframe].frame, keyframes[nextkeyframe].frame, keyframes[curkeyframe].variables[key], keyframes[nextkeyframe].variables[key])
