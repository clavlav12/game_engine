...
source = effect.CreateSource(initspeed=1.0, initdirection=0.0, initspeedrandrange=0.5, initdirectionrandrange=math.pi,
                             particlelife=1000, colour=(200, 255, 200), drawtype=PyIgnition.DRAWTYPE_BUBBLE, radius=4.0)
...
while True:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            source.CreateKeyframe(
                source.curframe + 1, pos=pygame.mouse.get_pos(), particlesperframe=1)
            source.CreateKeyframe(
                source.curframe + 2, pos=pygame.mouse.get_pos(), particlesperframe=0)
