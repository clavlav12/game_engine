...
class Frame(wx.Frame):
    def __init__(self, parent):
        ...
        self.source = self.effect.CreateSource((self.size[0] / 2, self.size[1]), initspeed=5.0, initdirection=0.0, initspeedrandrange=2.0,
                                               initdirectionrandrange=0.2, particlesperframe=10, particlelife=200, drawtype=PyIgnition.DRAWTYPE_POINT, colour=(255, 255, 200))
        self.grav = self.effect.CreateDirectedGravity(0.0, 0.2, [1, 0])
        self.gravslider = wx.Slider(
            self, wx.ID_ANY, 0, -50, 50, style=wx.SL_HORIZONTAL | wx.SL_LABELS)
    ...
    def OnSize(self, event):
        self.Layout()
        self.size = self.panel.GetSizeTuple()
        self.source.CreateKeyframe(
            self.source.curframe, pos=(self.size[0] / 2, self.size[1]))
    ...
    def OnScroll(self, event):
        self.grav.CreateKeyframe(self.grav.curframe, strength=float(
            self.gravslider.GetValue()) / 100.0)
