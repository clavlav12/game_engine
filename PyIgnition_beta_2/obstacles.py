...
class Obstacle:
    ...
    # Gets the force as a factor of maximum available force (0.0 - 1.0), determined by an inverse cube distance law
    ...
    def GetForce(self, pos, velocity):  # Gets the final (vector) force
        ...
        # An integer between 0.0 and 1.0 used to ensure force is maximised for head-on collisions and minimised for scrapes
        ...
        # Magnitude of the velocity - multiplied in the force equation
        velmag = magnitude(velocity)
...
class Circle(Obstacle):
    ...
    def Update(self):
        newvars = interpolate.InterpolateKeyframes(self.curframe, {'pos_x': self.pos[0], 'pos_y': self.pos[1], 'colour_r': self.colour[
                                                   0], 'colour_g': self.colour[1], 'colour_b': self.colour[2], 'bounce': self.bounce, 'radius': self.radius}, self.keyframes)
        self.pos = (newvars['pos_x'], newvars['pos_y'])
        self.colour = (newvars['colour_r'],
                       newvars['colour_g'], newvars['colour_b'])
    ...
    def GetForceFactor(self, pos):
        ...
        distcubed = (abs(pow(float(
            newpos[0] - self.pos[0]), 3.0)) + abs(pow(float(newpos[1] - self.pos[1]), 3.0)))
        if distcubed <= 1.0:
            return 1.0
    ...
    def CreateKeyframe(self, frame, pos=(None, None), colour=(None, None, None), bounce=None, radius=None, interpolationtype="linear"):
        keyframes.CreateKeyframe(self.keyframes, frame, {'pos_x': pos[0], 'pos_y': pos[1], 'colour_r': colour[0], 'colour_g': colour[1],
                                                         'colour_b': colour[2], 'bounce': bounce, 'radius': radius, 'interpolationtype': interpolationtype})

    def ConsolidateKeyframes(self):
        keyframes.ConsolidateKeyframes(self.keyframes, self.curframe, {
                                       'pos_x': self.pos[0], 'pos_y': self.pos[1], 'colour_r': self.colour[0], 'colour_g': self.colour[1], 'colour_b': self.colour[2], 'bounce': self.bounce, 'radius': self.radius})
...
class Rectangle(Obstacle):
    def __init__(self, pos, colour, bounce, width, height):
        ...
        self.CreateKeyframe(0, self.pos, self.colour,
                            self.bounce, self.width, self.height)

    def Draw(self, display):
        pygame.draw.rect(display, self.colour, pygame.Rect(
            self.pos[0] - self.halfwidth, self.pos[1] - self.halfheight, self.width, self.height))

    def Update(self):
        newvars = interpolate.InterpolateKeyframes(self.curframe, {'pos_x': self.pos[0], 'pos_y': self.pos[1], 'colour_r': self.colour[0], 'colour_g': self.colour[
                                                   1], 'colour_b': self.colour[2], 'bounce': self.bounce, 'width': self.width, 'height': self.height}, self.keyframes)
        self.pos = (newvars['pos_x'], newvars['pos_y'])
        self.colour = (newvars['colour_r'],
                       newvars['colour_g'], newvars['colour_b'])
        self.bounce = newvars['bounce']
    ...
    def GetResolved(self, pos):
        ...
        elif abs(float(pos[1] - self.pos[1]) / float(pos[0] - self.pos[0])) > (float(self.height) / float(self.width)):
            # Halfheight is multiplied by a normalised version of (pos[1] - self.pos[1]) - thus if (pos[1] - self.pos[1]) is negative, it should be subtracted as the point is in the upper triangle
            return [pos[0], self.pos[1] + (self.halfheight * ((pos[1] - self.pos[1]) / abs(pos[1] - self.pos[1])))]
    ...
    def CreateKeyframe(self, frame, pos=(None, None), colour=(None, None, None), bounce=None, width=None, height=None, interpolationtype="linear"):
        keyframes.CreateKeyframe(self.keyframes, frame, {'pos_x': pos[0], 'pos_y': pos[1], 'colour_r': colour[0], 'colour_g': colour[1],
                                                         'colour_b': colour[2], 'bounce': bounce, 'width': width, 'height': height, 'interpolationtype': interpolationtype})

    def ConsolidateKeyframes(self):
        keyframes.ConsolidateKeyframes(self.keyframes, self.curframe, {'pos_x': self.pos[0], 'pos_y': self.pos[1], 'colour_r': self.colour[
                                       0], 'colour_g': self.colour[1], 'colour_b': self.colour[2], 'bounce': self.bounce, 'width': self.width, 'height': self.height})
...
class BoundaryLine(Obstacle):
    ...
    def Update(self):
        newvars = interpolate.InterpolateKeyframes(self.curframe, {'pos_x': self.pos[0], 'pos_y': self.pos[1], 'colour_r': self.colour[0], 'colour_g': self.colour[
                                                   1], 'colour_b': self.colour[2], 'bounce': self.bounce, 'normal_x': self.normal[0], 'normal_y': self.normal[1]}, self.keyframes)
        self.pos = (newvars['pos_x'], newvars['pos_y'])
        self.colour = (newvars['colour_r'],
                       newvars['colour_g'], newvars['colour_b'])
        self.bounce = newvars['bounce']
    ...
    def CreateKeyframe(self, frame, pos=(None, None), colour=(None, None, None), bounce=None, normal=[None, None], interpolationtype="linear"):
        ...
        keyframes.CreateKeyframe(self.keyframes, frame, {'pos_x': pos[0], 'pos_y': pos[1], 'colour_r': colour[0], 'colour_g': colour[1],
                                                         'colour_b': colour[2], 'bounce': bounce, 'normal_x': normal[0], 'normal_y': normal[1], 'interpolationtype': interpolationtype})

    def ConsolidateKeyframes(self):
        keyframes.ConsolidateKeyframes(self.keyframes, self.curframe, {'pos_x': self.pos[0], 'pos_y': self.pos[1], 'colour_r': self.colour[
                                       0], 'colour_g': self.colour[1], 'colour_b': self.colour[2], 'bounce': self.bounce, 'normal_x': self.normal[0], 'normal_y': self.normal[1]})
