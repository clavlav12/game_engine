...
def CreateKeyframe(parentframes, frame, variables):
    ...
    try:
        oldkey = next(
            (keyframe for keyframe in parentframes if keyframe.frame == frame))
    ...
    if oldkey != None:
        ...
        for var in list(oldkey.variables.keys()):
            ...
            if (var not in newframe.variables or newframe.variables[var] == None) and (oldkey.variables[var] != None):
