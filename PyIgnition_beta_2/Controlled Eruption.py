...
starttext = starttextfont.render(
    "Press space to start", True, (255, 255, 255), (0, 0, 0))
starttextpos = ((400 - (starttext.get_width() / 2)),
                (300 - (starttext.get_height() / 2)))

# Background
background = PyIgnition.ParticleEffect(screen, (0, 0), (800, 600))
backgroundsource = background.CreateSource((10, 10), initspeed=5.0, initdirection=2.35619449, initspeedrandrange=2.0, initdirectionrandrange=1.0,
                                           particlesperframe=5, particlelife=125, drawtype=PyIgnition.DRAWTYPE_SCALELINE, colour=(255, 255, 255), length=10.0)
...
backgroundsource2 = background.CreateSource((790, 10), initspeed=5.0, initdirection=-2.35619449, initspeedrandrange=2.0, initdirectionrandrange=1.0,
                                            particlesperframe=0, particlelife=125, drawtype=PyIgnition.DRAWTYPE_SCALELINE, colour=(255, 255, 255), length=10.0)
...
backgroundsource2.CreateParticleKeyframe(
    100, colour=(0, 255, 255), length=10.0)
...
fireworksource = firework.CreateSource((10, 10), initspeed=8.0, initdirection=0.0, initspeedrandrange=2.0,
                                       initdirectionrandrange=math.pi, particlesperframe=0, particlelife=150, drawtype=PyIgnition.DRAWTYPE_IMAGE, image=sparkimg)
fireworkblast = background.CreateCircle(
    pos=(1000, 1000), colour=(0, 0, 0), bounce=1.5, radius=100.0)

# Ground-level bubbles
bubbles = PyIgnition.ParticleEffect(screen, (0, 0), (800, 600))
bubblesource = bubbles.CreateSource(initspeed=1.0, initdirection=0.0, initspeedrandrange=0.5, initdirectionrandrange=math.pi,
                                    particlesperframe=0, particlelife=200, colour=(200, 255, 200), drawtype=PyIgnition.DRAWTYPE_BUBBLE, radius=5.0, genspacing=5)
...
source = fire.CreateSource((150, 500), initspeed=2.0, initdirection=0.0, initspeedrandrange=1.0, initdirectionrandrange=0.5,
                           particlesperframe=10, particlelife=100, drawtype=PyIgnition.DRAWTYPE_CIRCLE, colour=(255, 200, 100), radius=3.0)
...
def Update():
    ...
    if (curframe > 200) and (curframe % 50 == 0):
        fireworksource.CreateKeyframe(
            fireworksource.curframe, particlesperframe=10)
        fireworksource.CreateKeyframe(
            fireworksource.curframe + 4, particlesperframe=0)
        firework.Update()
